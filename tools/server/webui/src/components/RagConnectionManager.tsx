...
export const RagConnectionManager: React.FC<RagConnectionManagerProps> = ({
  ...
  }, [localConfig.selected_rag_connection_name, localConfig.rag_connections]);
  ...
  const handleCreateSchema = async (connectionDetails?: RagConnection) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Schema creation successful: ${result.message}`);
      } else {
        const errorData = await response.json();
        console.log('connection to use', connectionToUse);
      ...
      console.error(
        'An error occurred while trying to create the RAG DB schema.'
      );
    }
  };
